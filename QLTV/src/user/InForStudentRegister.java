...
public class InForStudentRegister extends javax.swing.JFrame {
    ...
    private ArrayList<RegisterBook> listRBid;
    ...
    private void initComponents() {
        ...
        jPanel1.add(jLabel6, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0, 380, 280));
    ...
    public static ArrayList<RegisterBook> checkReBookId(ArrayList<RegisterBook> listrb,String bookId1) {
        ArrayList<RegisterBook> listtamthoi ;
        listtamthoi = new ArrayList<>();
        for (var e : listrb) {
            if (e.getBookId().equalsIgnoreCase(bookId1)) {
                listtamthoi.add(e);
    ...
    public boolean checksatus (ArrayList<RegisterBook> listrb){
        for(var e: listrb){
            if("chưa trả".equals(e.getGiveBookBack())){
                return true;
            }
        }
        return false;
            ...
            if (edit != -1) {
                ...
                Book book = checkBookId(bookId);
                 boolean flag = checkBookId(book);
                  ArrayList<RegisterBook> liststatus;
                  liststatus = new ArrayList<>();
                 liststatus =checkReBookId(listRBid, bookId);
    ...
    void edittbBook(Book book) {
    // Tìm chỉ mục của hàng có book.getBookId()
    int rowIndexToRemove = -1;
    for (int i = 0; i < tableModelBook.getRowCount(); i++) {
        if (tableModelBook.getValueAt(i, 1).equals(book.getBookId())) {
            rowIndexToRemove = i;
            break;
        }
    }
    ...
    if (rowIndexToRemove != -1) {
        // Nếu tìm thấy chỉ mục, thì xóa hàng đó
        tableModelBook.removeRow(rowIndexToRemove);

        // Thêm lại hàng mới
        var row = new Object[]{rowIndexToRemove + 1, book.getBookId(), book.getName(), book.getCategory(),
                book.getAuthor(), book.getProducer(), book.getPrice(), book.getAmount()};
        tableModelBook.insertRow(rowIndexToRemove, row);

        // Thông báo cho tableModelBook rằng dữ liệu đã thay đổi
        tableModelBook.fireTableDataChanged();
    } else {
        // Xử lý trường hợp không tìm thấy chỉ mục
        System.out.println("Không tìm thấy hàng với bookId: " + book.getBookId());
    }
}
    ...
    private void payBook(String bookId) {
        ...
        if (registerBook != null) {
            ...
            var book = checkBookId(registerBook.getBookId());
             var amount = book.getAmount() + registerBook.getAmountBook();
    ...
    private ArrayList<Book> checkSeach(String seach) {
        ...
        for (var e : listBook) {
            ...
            if (e.getBookId().equalsIgnoreCase(seach) || e.getName().equalsIgnoreCase(seach)
                    || e.getAuthor().equalsIgnoreCase(seach) || 
